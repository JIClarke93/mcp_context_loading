"""Benchmark script to compare context loading vs tool calling agents.

This script evaluates the context window and data loading pressure
for both agent approaches.
"""
...
async def count_tokens(text: str, model: str = "cl100k_base") -> int:
...
async def benchmark_context_agent(db: AsyncSession) -> dict:
    """Benchmark the context loading agent.
    ...
    # Load context without instantiating the agent
    from be.context.loader import load_full_context

    # Load all data
    context = await load_full_context(db, limit_per_entity=100)
    ...
    context_size_tokens = await count_tokens(context_json)

    # Count entities
    num_users = len(context.get("users", []))
    num_products = len(context.get("products", []))
    num_categories = len(context.get("categories", []))
    num_orders = len(context.get("orders", []))
    num_reviews = len(context.get("reviews", []))
    ...
    results = {
        ...
        "tool_calls": 0,
        "queries_to_db": 5,  # One query per entity type during load_full_context
    }
...
async def benchmark_tools_agent(db: AsyncSession) -> dict:
    """Benchmark the tool calling agent through simulated tool calls.
    ...
    # 1. List users (first page)
    users = await DISPATCH.users.list_users(db, skip=0, limit=10)
    users_json = json.dumps([u.model_dump() for u in users], default=str)
    users_tokens = await count_tokens(users_json)
    ...
    products_tokens = await count_tokens(products_json)
    tool_calls.append(
    ...
    # 3. List categories
    categories = await DISPATCH.categories.list_categories(db, skip=0, limit=10)
    categories_json = json.dumps([c.model_dump() for c in categories], default=str)
    categories_tokens = await count_tokens(categories_json)
    tool_calls.append(
    ...
    # 4. List orders
    orders = await DISPATCH.orders.list_orders(db, skip=0, limit=10)
    orders_json = json.dumps([o.model_dump() for o in orders], default=str)
    orders_tokens = await count_tokens(orders_json)
    ...
    # 5. List reviews
    reviews = await DISPATCH.reviews.list_reviews(db, skip=0, limit=10)
    reviews_json = json.dumps([r.model_dump() for r in reviews], default=str)
    reviews_tokens = await count_tokens(reviews_json)
    tool_calls.append(
    ...
    results = {
        ...
        "entities_loaded": {
            ...
            "orders": len(orders),
            "reviews": len(reviews),
            "total": len(users) + len(products) + len(categories) + len(orders) + len(reviews),
        },
    }
...
async def generate_visualizations(context_results: dict, tools_results: dict, output_dir: Path):
    ...
    fig.suptitle("Context Loading vs Tool Calling: Performance Comparison", fontsize=16, fontweight="bold")
    ...
    ax2 = axes[0, 1]
    entities = [context_results["entities_loaded"]["total"], tools_results["entities_loaded"]["total"]]
    ...
    # 3. Entity Breakdown (stacked bar chart)
    ax3 = axes[1, 0]
...
async def main():
    ...
    async with AsyncSessionLocal() as db:
        ...
        context_results = await benchmark_context_agent(db)

        # Benchmark tools agent
        tools_results = await benchmark_tools_agent(db)
    ...
    comparison = {
        ...
        "summary": {
            "token_reduction_pct": round(
                (
                    1
                    - tools_results["context_size_tokens"] / context_results["context_size_tokens"]
                )
                * 100,
                2,
            ),
    ...
    await generate_visualizations(context_results, tools_results, output_dir)
